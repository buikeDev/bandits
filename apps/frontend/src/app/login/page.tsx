...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.replace(requested?.startsWith('/') ? requested : '/account');
    } catch (cause) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-neutral-100 px-5 py-12">
      <section className="w-full max-w-md rounded-lg bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-neutral-600">Sign in to view your BANDIT account.</p>
        <form onSubmit={submit} className="mt-7 space-y-5">
          ...
          <label className="block text-sm font-semibold">Password
            <input name="password" type="password" autoComplete="current-password" required className="mt-2 w-full rounded border border-neutral-300 px-4 py-3 font-normal" />
          </label>
        ...
        <p className="mt-6 text-sm text-neutral-600">No account? <Link href="/register" className="font-bold text-black">Create one</Link></p>
      </section>
    </main>
  );
}
