...
export const getProducts = (query: URLSearchParams): Promise<PaginatedProductsDto> =>
  get(`/catalog/products?${query}`);
export const getProduct = (slug: string): Promise<ProductDetailDto> =>
  get(`/catalog/products/${encodeURIComponent(slug)}`);

export const getDesignOptions = (): Promise<{
  products: ProductDetailDto[];
  customizationFeeMinor: number;
}> => get('/catalog/design-options');
